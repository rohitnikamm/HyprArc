...
/// Lightweight TOML parser that handles the subset HyprArc needs:
/// key-value pairs, sections, and arrays of tables.
/// No external dependencies.
enum TOMLParser {

    /// Parse a TOML string into a HyprArcConfig.
    static func parse(_ toml: String) -> HyprArcConfig {
        ...
        for rawLine in toml.components(separatedBy: .newlines) {
            ...
            if line.hasPrefix("[[") && line.hasSuffix("]]") {
                ...
                if currentSection == "window-rules" {
                    config.windowRules.append(HyprArcConfig.WindowRule(appID: "", action: ""))
                }
                continue
            }
    ...
    private static func apply(
        key: String, value: String, section: String, config: inout HyprArcConfig
    ) {
        switch section {
        case "general":
            switch key {
