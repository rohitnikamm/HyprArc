...
struct TOMLParserTests {
    ...
    func parseDefaultConfig() {
        let config = TOMLParser.parse(HyprArcConfig.defaultTOML)
    ...
    func serializeWithWorkspaceOnly() {
        var config = HyprArcConfig()
        config.windowRules = [
            HyprArcConfig.WindowRule(appID: "com.apple.mail", action: "", workspace: 3),
        ]
    ...
    func serializeWithActionAndWorkspace() {
        var config = HyprArcConfig()
        config.windowRules = [
            HyprArcConfig.WindowRule(appID: "com.spotify.client", action: "float", workspace: 2),
        ]
        let toml = TOMLSerializer.serialize(config)
    ...
    func roundTripWorkspace() {
        var original = HyprArcConfig()
        original.windowRules = [
            HyprArcConfig.WindowRule(appID: "com.apple.mail", action: "", workspace: 5),
            HyprArcConfig.WindowRule(appID: "com.spotify.client", action: "float", workspace: 2),
        ]
